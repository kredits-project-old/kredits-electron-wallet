...
export class WalletRPC {
                ...
                portscanner.checkPortStatus(this.port, this.hostname).catch(e => "closed").then(status => {
                    if (status === "closed") {
                        if (process.platform === "win32") {
                            this.walletRPCProcess = child_process.spawn(path.join(__ryo_bin, "kredits-wallet-rpc.exe"), args)
                        } else {
                            this.walletRPCProcess = child_process.spawn(path.join(__ryo_bin, "kredits-wallet-rpc"), args, {
                                detached: true
                            })
                        }
    ...
    handle (data) {
        ...
        case "restore_view_wallet":
            // TODO: Decide if we want this for kredits
    ...
    stake (password, amount, service_node_key, destination) {
        crypto.pbkdf2(password, this.auth[2], 1000, 64, "sha512", (err, password_hash) => {
            ...
            amount = parseFloat(amount).toFixed(9) * 1e8
    ...
    listWallets (legacy = false) {
        ...
        if (legacy) {
            ...
            if (os.platform() == "win32") {
                legacy_paths = ["C:\\ProgramData\\kredits"]
            } else {
                legacy_paths = [path.join(os.homedir(), "kredits")]
            }
