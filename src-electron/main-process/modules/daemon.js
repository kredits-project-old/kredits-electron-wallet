...
export class Daemon {
    constructor (backend) {
        ...
        this.PIVOT_BLOCK_TIMESTAMP = 1555529414
    ...
    checkVersion () {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let kreditsd_path = path.join(__ryo_bin, "kreditsd.exe")
                let kreditsd_version_cmd = `"${kreditsd_path}" --version`
                if (!fs.existsSync(kreditsd_path)) { resolve(false) }
                child_process.exec(kreditsd_version_cmd, (error, stdout, stderr) => {
                    if (error) { resolve(false) }
                    resolve(stdout)
                })
            } else {
                let kreditsd_path = path.join(__ryo_bin, "kreditsd")
                let kreditsd_version_cmd = `"${kreditsd_path}" --version`
                if (!fs.existsSync(kreditsd_path)) { resolve(false) }
                child_process.exec(kreditsd_version_cmd, { detached: true }, (error, stdout, stderr) => {
                    if (error) { resolve(false) }
                    resolve(stdout)
                })
            }
    ...
    start (options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            args.push("--log-file", path.join(dirs[net_type], "logs", "kreditsd.log"))
            ...
            portscanner.checkPortStatus(this.port, this.hostname).catch(e => "closed").then(status => {
                if (status === "closed") {
                    if (process.platform === "win32") {
                        this.daemonProcess = child_process.spawn(path.join(__ryo_bin, "kreditsd.exe"), args)
                    } else {
                        this.daemonProcess = child_process.spawn(path.join(__ryo_bin, "kreditsd"), args, {
                            detached: true
                        })
                    }
