...
<div class="tx-list">
    ...
    <template v-else>
        <q-infinite-scroll :handler="loadMore" ref="scroller">
            <q-list link no-border :dark="theme=='dark'" class="kredits-list tx-list">
                <q-item class="kredits-list-item transaction" v-for="(tx, index) in tx_list_paged" :key="`${tx.txid}-${tx.type}`"
                    ...
                    <q-item-main class="main">
                        <q-item-tile class="amount" label>
                            <FormatKredits :amount="tx.amount" />
                        </q-item-tile>
